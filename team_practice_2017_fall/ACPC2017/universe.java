...
public class universe {
    ...
    private static void dfs(ArrayList<Integer>[] gragh, int src, int det, int mode)
    {
        ArrayDeque<path> queue = new ArrayDeque<path>();
        queue.add(new path(src, 0));
        boolean[][] list = new boolean[gragh.length][2001];
        list[0][0] = true;
        
        while(!queue.isEmpty())
        {
            path current = queue.removeLast();
            list[current.det][current.length] = true;
